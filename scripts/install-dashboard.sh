...
# ═════════════════════════════════════════════════════════════════════════
heading "9/10 · Cron — auto-sync + nightly backup"
...
# Encrypted nightly backup at 02:30 server time.
30 2 * * * root cd ${INSTALL_DIR} && /bin/bash scripts/backup-all.sh >> /var/log/sontoloyo-backup.log 2>&1
EOF
  chmod 644 "${CRON_FILE}"
...
echo "  ${C_BOLD}Backup cron${C_RESET}   : daily 02:30 → /root/sontoloyo-backups/"
...
echo "       • Set SONTOLOYO_BACKUP_PASSPHRASE in .env to enable encryption"
